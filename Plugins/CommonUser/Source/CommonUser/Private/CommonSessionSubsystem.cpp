...
UCommonSession_HostSessionRequest* UCommonSessionSubsystem::CreateAdvancedOnlineHostSessionRequest(FPrimaryAssetId MapID, FString ModeNameForAdvertisement, int32 MaxPlayers)
{
	...
	Result->MapID = MapID;
	Result->ModeNameForAdvertisement = ModeNameForAdvertisement;
	Result->MaxPlayerCount = MaxPlayers;
...
void UCommonSessionSubsystem::CleanUpSessionsOSSv1()
{
	...
	// TODO: Workaround for GetSessionState not returning Pending for clients after joining a session
	else if (EOnlineSessionState::NoSession == SessionState)
	{
		FNamedOnlineSession* Session = Sessions->GetNamedSession(NAME_GameSession);
		if (Session && EOnlineSessionState::NoSession == Session->SessionState)
		{
			UE_LOG(LogCommonSession, Log, TEXT("Client Session State Workaround: Destroying session on return to main menu"));
			Sessions->DestroySession(NAME_GameSession);
		}
	}
}
