...
public:
	...
	/** Creates a host session request with default options for online games, this can be modified after creation */
	UFUNCTION(BlueprintCallable, Category = Session)
	virtual UCommonSession_HostSessionRequest* CreateAdvancedOnlineHostSessionRequest(UPARAM(meta = (AllowedTypes = "Map")) FPrimaryAssetId MapID, FString ModeNameForAdvertisement, int32 MaxPlayers = 8);
