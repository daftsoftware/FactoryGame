...
#include "FGSettingKeyboardInput.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(FGSettingKeyboardInput)

class ULocalPlayer;

#define LOCTEXT_NAMESPACE "FGSettings"

namespace FG::ErrorMessages
{
	static const FText UnknownMappingName = LOCTEXT("FGErrors_UnknownMappingName", "Unknown Mapping");
}


UFGSettingKeyboardInput::UFGSettingKeyboardInput()
{
	bReportAnalytics = false;
}

FText UFGSettingKeyboardInput::GetSettingDisplayName() const
{
	...
	return FG::ErrorMessages::UnknownMappingName;
}

FText UFGSettingKeyboardInput::GetSettingDisplayCategory() const
{
	if (const FKeyMappingRow* Row = FindKeyMappingRow())
	{
		if (Row->HasAnyMappings())
...
const FKeyMappingRow* UFGSettingKeyboardInput::FindKeyMappingRow() const
{
...
UEnhancedPlayerMappableKeyProfile* UFGSettingKeyboardInput::FindMappableKeyProfile() const
{
...
UEnhancedInputUserSettings* UFGSettingKeyboardInput::GetUserSettings() const
{
	if(UFGLocalPlayer* FGLocalPlayer = Cast<UFGLocalPlayer>(LocalPlayer))
	{
		// Map the key to the player key profile
		if (UEnhancedInputLocalPlayerSubsystem* System = FGLocalPlayer->GetSubsystem<UEnhancedInputLocalPlayerSubsystem>())
		{
...
void UFGSettingKeyboardInput::OnInitialized()
{
...
void UFGSettingKeyboardInput::InitializeInputData(const UEnhancedPlayerMappableKeyProfile* KeyProfile, const FKeyMappingRow& MappingData, const FPlayerMappableKeyQueryOptions& InQueryOptions)
{
	check(KeyProfile);
...
FText UFGSettingKeyboardInput::GetKeyTextFromSlot(const EPlayerMappableKeySlot InSlot) const
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
...
FText UFGSettingKeyboardInput::GetPrimaryKeyText() const
{
	return GetKeyTextFromSlot(EPlayerMappableKeySlot::First);
}

FText UFGSettingKeyboardInput::GetSecondaryKeyText() const
{
	return GetKeyTextFromSlot(EPlayerMappableKeySlot::Second);
}
PRAGMA_ENABLE_DEPRECATION_WARNINGS

void UFGSettingKeyboardInput::ResetToDefault()
{	
	if (UEnhancedInputUserSettings* Settings = GetUserSettings())
	{
...
void UFGSettingKeyboardInput::StoreInitial()
{
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{
		if(const FKeyMappingRow* Row = FindKeyMappingRow())
...
void UFGSettingKeyboardInput::RestoreToInitial()
{
...
bool UFGSettingKeyboardInput::ChangeBinding(int32 InKeyBindSlot, FKey NewKey)
{
...
void UFGSettingKeyboardInput::GetAllMappedActionsFromKey(int32 InKeyBindSlot, FKey Key, TArray<FName>& OutActionNames) const
{	
	if (const UEnhancedPlayerMappableKeyProfile* Profile = FindMappableKeyProfile())
	{		
...
bool UFGSettingKeyboardInput::IsMappingCustomized() const
{
	bool bResult = false;
