...
#include "Settings/FGPerformanceSettings.h"
#include "Settings/FGGameSettingRegistry.h"

#include "CustomSettings/FGSettingAction_SafeZoneEditor.h"
#include "CustomSettings/FGSettingValueDiscrete_OverallQuality.h"
#include "CustomSettings/FGSettingValueDiscrete_Resolution.h"
...
enum class EFGFramePacingEditCondition
{
	EnableIf,
	DisableIf
};
...
class FFGGameSettingEditCondition_FramePacingMode : public FGameSettingEditCondition
{
public:
	FFGGameSettingEditCondition_FramePacingMode(EFGFramePacingMode InDesiredMode, EFGFramePacingEditCondition InMatchMode = EFGFramePacingEditCondition::EnableIf)
	...
	{
		const EFGFramePacingMode ActualMode = UFGPlatformSpecificRenderingSettings::Get()->FramePacingMode;
		
		const bool bMatches = (ActualMode == DesiredMode);
		const bool bMatchesAreBad = (MatchMode == EFGFramePacingEditCondition::DisableIf);
...
private:
	EFGFramePacingMode DesiredMode;
	EFGFramePacingEditCondition MatchMode;
};
...
public:
	...
	virtual void GatherEditState(const ULocalPlayer* InLocalPlayer, FGameSettingEditableState& InOutEditState) const override
	{
		if (!UFGPlatformSpecificRenderingSettings::Get()->bSupportsGranularVideoQualitySettings)
		{
	...
	virtual void SettingChanged(const ULocalPlayer* LocalPlayer, UGameSetting* Setting, EGameSettingChangeReason Reason) const override
	{
		...
		const UFGLocalPlayer* FGLocalPlayer = CastChecked<UFGLocalPlayer>(LocalPlayer);
		FGLocalPlayer->GetLocalSettings()->ApplyScalabilitySettings();
	}
...
UGameSettingCollection* UFGGameSettingRegistry::InitializeVideoSettings(UFGLocalPlayer* InLocalPlayer)
{
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			UFGSettingValueDiscrete_Resolution* Setting = NewObject<UFGSettingValueDiscrete_Resolution>();
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UFGSettingsShared>()->GetColorBlindMode());
			Setting->AddEnumOption(EFGColorBlindMode::Off, LOCTEXT("ColorBlindRotatorSettingOff", "Off"));
			Setting->AddEnumOption(EFGColorBlindMode::Deuteranope, LOCTEXT("ColorBlindRotatorSettingDeuteranope", "Deuteranope"));
			Setting->AddEnumOption(EFGColorBlindMode::Protanope, LOCTEXT("ColorBlindRotatorSettingProtanope", "Protanope"));
			Setting->AddEnumOption(EFGColorBlindMode::Tritanope, LOCTEXT("ColorBlindRotatorSettingTritanope", "Tritanope"));
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UFGSettingsShared>()->GetColorBlindStrength());
		...
		//----------------------------------------------------------------------------------
		{
			UFGSettingAction_SafeZoneEditor* Setting = NewObject<UFGSettingAction_SafeZoneEditor>();
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			for (const FFGQualityDeviceProfileVariant& Variant : PlatformSettings->UserFacingDeviceProfileOptions)
			{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetCustomAction([](ULocalPlayer* LocalPlayer)
			{
				...
				FGLocalPlayer->GetLocalSettings()->RunAutoBenchmark(bImmediatelySaveState);
			});
			...
			Setting->AddEditCondition(MakeShared<FWhenCondition>([](const ULocalPlayer* LocalPlayer, FGameSettingEditableState& InOutEditState)
			{
				...
				const UFGLocalPlayer* FGLocalPlayer = CastChecked<UFGLocalPlayer>(LocalPlayer);
				const bool bCanBenchmark = FGLocalPlayer->GetLocalSettings()->CanRunAutoBenchmark();
		...
		//----------------------------------------------------------------------------------
		{
			UFGSettingValueDiscrete_OverallQuality* Setting = NewObject<UFGSettingValueDiscrete_OverallQuality>();
			...
			Setting->AddEditCondition(MakeShared<FFGGameSettingEditCondition_FramePacingMode>(EFGFramePacingMode::ConsoleStyle, EFGFramePacingEditCondition::DisableIf));
	...
	////////////////////////////////////////////////////////////////////////////////////
	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->AddEditCondition(MakeShared<FFGGameSettingEditCondition_FramePacingMode>(EFGFramePacingMode::DesktopStyle));
...
void AddFrameRateOptions(UGameSettingValueDiscreteDynamic_Number* Setting)
{
	...
	for (int32 Rate : GetDefault<UFGPerformanceSettings>()->DesktopFrameRateLimits)
	{
...
void UFGGameSettingRegistry::InitializeVideoSettings_FrameRates(UGameSettingCollection* Screen, UFGLocalPlayer* InLocalPlayer)
{
	//----------------------------------------------------------------------------------
	{
		UGameSettingValueDiscreteDynamic_Number* Setting = NewObject<UGameSettingValueDiscreteDynamic_Number>();
		...
		Setting->SetDefaultValue(GetDefault<UFGSettingsLocal>()->GetFrameRateLimit_OnBattery());
	...
	//----------------------------------------------------------------------------------
	{
		...
		Setting->SetDefaultValue(GetDefault<UFGSettingsLocal>()->GetFrameRateLimit_InMenu());
		Setting->AddEditCondition(MakeShared<FFGGameSettingEditCondition_FramePacingMode>(EFGFramePacingMode::DesktopStyle));
	...
	//----------------------------------------------------------------------------------
	{
		...
		Setting->SetDefaultValue(GetDefault<UFGSettingsLocal>()->GetFrameRateLimit_WhenBackgrounded());
		Setting->AddEditCondition(MakeShared<FFGGameSettingEditCondition_FramePacingMode>(EFGFramePacingMode::DesktopStyle));
	...
	//----------------------------------------------------------------------------------
	{
		...
		Setting->SetDefaultValue(GetDefault<UFGSettingsLocal>()->GetFrameRateLimit_Always());
		Setting->AddEditCondition(MakeShared<FFGGameSettingEditCondition_FramePacingMode>(EFGFramePacingMode::DesktopStyle));
